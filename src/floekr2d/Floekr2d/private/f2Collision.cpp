...
namespace Floekr2d
{
    namespace core
    {
        bool f2CircletoCircle(f2Manifold* m, f2CircleShape* A, f2CircleShape* B)
        {
            ...
            float32 distance = f2Sqrt(distanceSquare);
            // 圆AB重合
            if(distance == 0)
            {
                // 随机选择一个方向作为分离方向(单位向量)
        ...
        f2Vector2f f2GetSupport(f2PolygonShape* shape, f2Vector2f& dir)
        {
        ...
        float32 f2FindMaxSeparation(uint32* faceIndex, f2PolygonShape* A, f2PolygonShape* B)
        {
            ...
            for(uint32 i = 0; i < A->getVertexCount(); i++)
            {
                ...
                // 在A面法线相反方向上找在B上的支点
                // --!这里并非多此一举 而是因为Xcode的临时对象生成和VS有不同
                // --!倘若这里直接getSupport(B, -n)那么VS会自动产生临时对象 而Xcode不会
                f2Vector2f n1 = -n;
                f2Vector2f supportPoint = f2GetSupport(B, n1);
