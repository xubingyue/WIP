    ...
    namespace core
    {
        ...
        void f2ContactController::broadPhase(f2List<f2Body*>& bodyList)
        {
            ...
            for(int32 i = 0; i < bodyList.size(); i++)
            {
                ...
                // 只有支持碰撞检测且不为动态物体的 是不会参与碰撞检测的
                if(A->isDynamic() || !A->isCollisionDetectable())
                    continue;
                ...
                for(int32 j = i + 1; j < bodyList.size(); j++)
                {
                    ...
                    if(B->isDynamic() || !B->isCollisionDetectable())
                        continue;

                    // 两者为静态物体 或 运动型物体不参与碰撞检测
        ...
        void f2ContactController::narrowPhase(float32& dt, math::f2Vector2f& gravity)
        {
            ...
            for(int32 i = 0; i < contactList.size(); i++)
            {
                ...
                else
                {
                    ...
                    // 更新AB二者的碰撞队列
                    // --!实质为通过挂钩友元实现 为了防止外部篡改内部碰撞逻辑
                    A->addCollideBody(B);
                    B->addCollideBody(A);

                    // 实际恢复系数为较小的一方
                    m->e = f2Min(A->restitution, B->restitution);

                    // 摩擦系数为两者平均
                    m->staticFriction = f2Sqrt(A->staticFriction * B->staticFriction);
                    m->dynamicFriction = f2Sqrt(A->dynamicFriction * B->dynamicFriction);
