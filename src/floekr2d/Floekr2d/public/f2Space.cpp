...
namespace Floekr2d
{
    ...
    void f2Space::init()
    {
        ...
        numOfIteration = F2_DEFAULT_ITERATION;
    }
    ...
    {
        for(uint32 iteration = 0; iteration < numOfIteration; iteration++)
        {
            // 对所有确认的碰撞对作用冲量
            for(int32 i = 0; i < contactList.size(); i++)
            {
                ...
                // 二者之中只要有一个关闭了碰撞应答 那么本次碰撞反馈就无需进行
                if(!A->isCollisionFeedback() || !B->isCollisionFeedback())
                    continue;
                ...
                for(int32 j = 0; j < m->touchPointsNum; j++)
                {
                    ...
                    // 摩擦力冲量
                    // 冲量计算方式:将原先的impluse计算方法中的n替换为垂直于normal的tangent
                    // --!之所以不书写vAB * normal主要是为了阅读方便
                    math::f2Vector2f tangent = vAB - normal * (vAB.dot(normal));
                    tangent.normalize();

                    float32 jt = -vAB*tangent;
                    jt /= denominator;
                    jt /= (float32)m->touchPointsNum;

                    // 小摩擦力冲量跳过计算
                    if(f2Equals(jt, 0.0f))
                        return;

                    math::f2Vector2f frictionImpulse;
                    // 滑动摩擦力小于等于最大静摩擦力
                    if(std::abs(jt) < impulseValue * m->staticFriction)
                        // jt在tangent向量上的分量
                        frictionImpulse = tangent * jt;
                    else
                        // 最大静摩擦
                        frictionImpulse = tangent * -impulseValue * m->dynamicFriction;
    ...
    void f2Space::penetrationCorrection()
    {
        for(int32 i = 0; i < contactList.size(); i++)
        {
            ...
            // 二者之中只要有一个关闭了碰撞应答 那么本次深度修正就无需进行
            if(!A->isCollisionFeedback() || !B->isCollisionFeedback())
                continue;
            ...
            // 质量大的一方修正的较小
            math::f2Vector2f correction = (f2Max(m->penetration - k_slop, 0.0f) / (inverseMassA + inverseMassB)) * m->normal * percent;
    ...
    void f2Space::setGravity(float32 x, float32 y)
    {
        setGravity(math::f2Vector2f(x, y));
    }
    ...
    void f2Space::setIteration(uint32 numOfIteration)
    {
        if(numOfIteration > FLT_MAX)
            numOfIteration = F2_DEFAULT_ITERATION;
        else
