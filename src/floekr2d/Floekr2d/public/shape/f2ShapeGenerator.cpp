...
namespace Floekr2d
{
    ...
    {
        // 具体类型
        f2CircleShape* shapeMem = (f2CircleShape*) circleShapeAllocator->alloc(sizeof(f2CircleShape));

        f2CircleShape* shape = (f2CircleShape*) shapeMem;
        // 初始化其具体类型 因为自定义的内存分配器不会调用构造函数
        new (shape) f2CircleShape();
        // 以下调用显示构造函数的方式只能在VS上通过
    ...
    {
        // 具体类型
        f2PolygonShape* shapeMem = (f2PolygonShape*) polygonShapeAllocator->alloc(sizeof(f2PolygonShape));

        f2PolygonShape* shape = (f2PolygonShape*) shapeMem;
        // 初始化其具体类型 因为自定义的内存分配器不会调用构造函数
        new (shape) f2PolygonShape();
        // 以下调用显示构造函数的方式只能在VS上通过
