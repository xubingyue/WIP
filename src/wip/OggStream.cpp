...
OggStream::OggStream() : WIPSoundBase(true)
{
	loop=false;
	bstop = false;
	in_list = false;
	...
	if(error!=AL_FALSE)
	{
		switch(error)
		{
			case(AL_INVALID_VALUE):
				g_logger->debug_print(WIP_ERROR,"Invalid value for gain");
			break;
			default:			
				g_logger->debug_print(WIP_ERROR,"Error trying to set gain!");
			break;
		}
	}
}
...
void OggStream::check()
{
	...
	if(error != AL_NO_ERROR)
		g_logger->debug_print(WIP_ERROR,"OpenAL error was raised.");
}
