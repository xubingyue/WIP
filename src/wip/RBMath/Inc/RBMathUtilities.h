...
struct RBBaseMath
{
	...
	static FORCEINLINE f32 pow(f32 a,f32 b){::powf(a,b);}
...
struct RBMath : public RBBaseMath
{
	...
	//求指定范围内的随机数，取到最大最小值
	static FORCEINLINE i32 get_rand_range_i(i32 min, i32 max)
	{
		i32 _range = (max - min) + 1;
		return min + get_rand_i(_range);
	}

	static FORCEINLINE f32 get_rand_range_f(f32 min, f32 max)
	{
		return min + (max - min) * rand_f();
	}


	static INLINE f64 get_gaussian_random()
	{
