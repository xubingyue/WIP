#pragma once
#include "Utilities/WPHash_Map.h"
...
struct SoundData
{
	int frequency;
	int size;
	unsigned int buffer_index;//there is no source_index
	int al_format;
	//by now i don't know it,just give it a value
	ALboolean loop;
};
...
private:
	...
	typedef Map<std::string,ResHandler*> file2ptr_map;
	typedef Map<ResHandler*,int> ref_map;
	file2ptr_map _map;
	ref_map _ref;
};
